...
class TestModuleExecution(unittest.TestCase):
    def test_tree_visualizer_module_runs_help(self):
        ...
        proc = subprocess.run([sys.executable, '-m', 'import_tools.tree_visualizer', '--help'], capture_output=True, text=True)
        self.assertEqual(proc.returncode, 0, f"Module failed to run: {proc.stderr}")
