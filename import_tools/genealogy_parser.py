...
def create_database(db_name='data/genealogy.db'):
    ...
    cursor.execute(
        'CREATE INDEX IF NOT EXISTS idx_relationships_child_family ON relationships(child_id, family_tree)')
    cursor.execute(
    ...
    cursor.execute(
        'CREATE INDEX IF NOT EXISTS idx_relationships_child_family_reltype_parent ON relationships(child_id, family_tree, relationship_type, parent_id)')

    # Index on individual_tree_instances used by import-time lookups (family_tree, old_id)
    cursor.execute(
        'CREATE INDEX IF NOT EXISTS idx_iti_family_old ON individual_tree_instances(family_tree, old_id)')

    # Index on individuals for name+dob lookups (used when trying alternate instances by name and DOB)
    cursor.execute(
        'CREATE INDEX IF NOT EXISTS idx_individuals_name_dob ON individuals(canonical_name, date_of_birth)')
